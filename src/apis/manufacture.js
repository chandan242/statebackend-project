...
export const checkUserManufacture = async (username) => {
    const url = `http://www.thexyz.biz:8087/api/Manufacturer/checkuser?UserName=${username}`;
    let token = localStorage.getItem("token");
    try {
...
export const addUserManufacture = async (data) =>{
    ...
    const response = await fetch(`${BaseURL}/Manufacturer/createuser`, requestOptions);
    const responseParsed = await response.json()
    console.log(responseParsed, "responseParsed")
    return responseParsed;
}
